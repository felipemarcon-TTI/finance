...
from app.config import SLIPPAGE_PCT, FEE_PCT, DEFAULT_SL_PCT, DEFAULT_TP_PCT
...
def execute_trade(signal, portfolio):
    ...
    if action=="SELL":
        pos["stop_loss"]=entry*(1+DEFAULT_SL_PCT)
        pos["take_profit"]=entry*(1-DEFAULT_TP_PCT)
    trade=database.open_trade(
        portfolio_id=portfolio["id"], symbol=symbol, timeframe=signal["timeframe"],
        action=action, entry_price=entry, stop_loss=pos["stop_loss"],
...
def check_and_close_position(trade):
    ...
    closed=database.close_trade(trade["id"],current,status,pnl_usdt,
                                 pnl_usdt/eur_rate,(pnl_usdt/(entry*qty))*100,exit_fees)
    rs=database.get_risk_state()
